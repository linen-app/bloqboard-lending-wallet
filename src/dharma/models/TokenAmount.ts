...
function registryDataForSymbol(symbol: string): RegistryData {
    const registryData = Tokens.networks.mainnet.find(tokenObject => tokenObject.symbol === symbol);
...
export class TokenAmount {
    ...
    constructor(amount: number | BigNumber, symbol: string) {
        this.data = registryDataForSymbol(symbol);
        this.rawAmount = TokenAmount.convertToRaw(new BigNumber(amount.toString()), this.data.numDecimals); // .round();
    }
